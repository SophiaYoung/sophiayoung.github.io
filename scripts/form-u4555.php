...
{
	$to = 'shiyi1110@gmail.com';
	$subject = 'Contact me 送出';
	...
	$message .= '<tr><td valign="top" style="background-color: #ffffff;"><b>Name:</b></td><td>' . htmlentities($_REQUEST["custom_U4562"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</td></tr>';
	...
	$message .= '<tr><td valign="top" style="background-color: #ffffff;"><b>Message:</b></td><td>' . htmlentities($_REQUEST["custom_U4556"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</td></tr>';

	$message .= '</table><br/><br/>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">表單由此網站送出: ' . htmlentities($_SERVER["SERVER_NAME"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</div>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">訪客 IP 位址: ' . htmlentities($_SERVER["REMOTE_ADDR"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</div>';
	$message .= '</body></html>';
	$message = cleanupMessage($message);
	
	$formEmail = cleanupEmail($_REQUEST['Email']);
	$headers = 'From:  shiyi1110@gmail.com' . "\r\n" . 'Reply-To: ' . $formEmail .  "\r\n" .'X-Mailer: Adobe Muse 5.0.704 with PHP/' . phpversion() . "\r\n" . 'Content-type: text/html; charset=utf-8' . "\r\n";
